  ...
  return (
    <div style={{ minHeight: '100vh', background: '#fff', fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      <style>{`
        ...
        .stat-down {
          animation: fadeInLeft 0.6s ease 1s both;
        }

        /* Hero elements */
        .hero-badge { animation: fadeInUp 0.6s ease 0.2s both; }
        .hero-h1 { animation: fadeInUp 0.7s ease 0.35s both; }
        .hero-p { animation: fadeInUp 0.7s ease 0.5s both; }
        .hero-buttons { animation: fadeInUp 0.7s ease 0.65s both; }
        .hero-stats { animation: fadeInUp 0.7s ease 0.8s both; }
        .hero-image { animation: rotateIn 0.8s ease 0.4s both; }

        /* Social proof fade in */
        .social-proof { animation: fadeInUp 0.6s ease 0.2s both; }
      ...
      <section style={{ padding: '72px 40px 96px', background: 'linear-gradient(180deg, #f7f9ff 0%, #fff 60%)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1.1fr', gap: 72, alignItems: 'center' }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            {/* Floating stat cards */}
            <div style={{ position: 'absolute', bottom: -16, left: -20, background: '#fff', borderRadius: 12, padding: '14px 20px', boxShadow: '0 8px 24px rgba(0,0,0,0.1)', border: '1px solid rgba(0,0,0,0.05)', display: 'flex', alignItems: 'center', gap: 12 }} className="floating-stat">
            ...
            <div style={{ position: 'absolute', top: 24, right: -16, background: '#fff', borderRadius: 12, padding: '12px 16px', boxShadow: '0 8px 24px rgba(0,0,0,0.1)', border: '1px solid rgba(0,0,0,0.05)', display: 'flex', alignItems: 'center', gap: 10 }} className="floating-stat-delayed">
      ...
      {/* Features */}
      <section id="features-section" className={`animate-on-scroll ${visibleSections.has('features-section') ? 'visible' : ''}`} style={{ padding: '88px 40px', background: '#fff' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }}>
            {features.map((f, i) => (
              <div key={f.title} className="feature-card animate-on-scroll" style={{ padding: 28, background: '#fff', border: '1px solid #f0f0f0', borderRadius: 12, cursor: 'pointer', animationDelay: `${i * 0.1}s' }}>
      ...
      {/* Testimonials */}
      <section id="testimonials-section" className={`animate-on-scroll ${visibleSections.has('testimonials-section') ? 'visible' : ''}`} style={{ padding: '80px 40px', background: '#f7f9ff' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
            {testimonials.map((t, i) => (
              <div key={t.name} className="testimonial-card animate-on-scroll" style={{ background: '#fff', border: '1px solid #f0f0f0', borderRadius: 14, padding: 28, animationDelay: `${i * 0.15}s' }}>
      ...
      {/* How it works */}
      <section id="how-it-works-section" className={`animate-on-scroll ${visibleSections.has('how-it-works-section') ? 'visible' : ''}`} style={{ padding: '88px 40px', background: '#fff' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 48 }}>
            {[
              ...
              <div key={step.n} style={{ textAlign: 'center' }}>
                <div className="step-number" style={{ width: 60, height: 60, borderRadius: '50%', background: 'linear-gradient(135deg, #1677ff 0%, #4096ff 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px', fontSize: 22, fontWeight: 800, color: '#fff', boxShadow: '0 8px 24px rgba(22,119,255,0.3)', cursor: 'pointer' }}>
                  {step.n}
                </div>
      ...
      {/* Pricing */}
      <section id="pricing-section" className={`animate-on-scroll ${visibleSections.has('pricing-section') ? 'visible' : ''}`} style={{ padding: '88px 40px', background: '#fafafa' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24, alignItems: 'start' }}>
            {plans.map((plan, i) => (
              <div key={plan.name} className="plan-card animate-on-scroll" style={{ background: '#fff', border: plan.popular ? '2px solid #1677ff' : '1px solid #f0f0f0', borderRadius: 16, padding: 32, position: 'relative', boxShadow: plan.popular ? '0 12px 40px rgba(22,119,255,0.12)' : '0 2px 8px rgba(0,0,0,0.04)', animationDelay: `${i * 0.1}s' }}>
                {plan.popular && (
      ...
      <section style={{ padding: '88px 40px', background: 'linear-gradient(135deg, #1677ff 0%, #4096ff 100%)' }}>
        <div style={{ maxWidth: 680, margin: '0 auto', textAlign: 'center' }}>
          ...
          <Link href="/auth/signup" className="cta-button" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '16px 40px', background: '#fff', color: '#1677ff', borderRadius: 8, fontSize: 16, fontWeight: 700, textDecoration: 'none', boxShadow: '0 8px 24px rgba(0,0,0,0.15)' }}>
